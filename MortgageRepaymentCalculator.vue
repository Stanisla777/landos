...
const config = {
  ...
  optimization: {
    minimize: false, // Будет переопределено для production
    splitChunks: {
      cacheGroups: {
            ...
            use: [
              {
                ...
                options: {
                  sources: false,
                  minimize: false, // Отключаем минификацию HTML на уровне html-loader
                  esModule: false
                }
              },
              {
                ...
                options: {
                  pretty: true, // Делаем HTML красивым
            ...
            options: {
              ...
              postcssOptions: {
                plugins: [
                  ...
                  // CSS минифицируем ТОЛЬКО в production
                  process.env.NODE_ENV === 'production' && require('cssnano')({ 
                    preset: ['default', { 
                      discardComments: { removeAll: true }, // удаляем комментарии
                      normalizeWhitespace: true // минифицируем
                    }] 
                  })
                ].filter(Boolean)
              }
  ...
  plugins: [
    ...
    // Генерируем HTML для всех pug файлов через HtmlRspackPlugin
    ...pugFiles.map(file => {
      ...
      return new rspack.HtmlRspackPlugin({
        ...
        inject: 'body',
        scriptLoading: 'defer',
        // 👇 HTML НИКОГДА не минифицируем - нужно бэкендеру
        minify: false,
        // 👇 Отключаем строгую проверку HTML
        templateParameters: {
          pretty: true,
...
module.exports = (env, argv) => {
  ...
  if (isProduction) {
    ...
    // 👇 ТОЛЬКО JS минифицируем в production
