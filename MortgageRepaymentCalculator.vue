      ...
      .block-grid-area__col.js--block-grid-area
        ...
        .calc-tax-deduc-new__block.mobile-padding.mor-rep-calculators__block-schedule.desctop.grey.js--container-block(
          ...
          .mor-rep-calculators__shedule-footer-share
            ...
            .btn_s.black(@click="exportToPDF") Скачать расчет
...
import {logoBase64, iconSymbolBase64} from '../pdf/image-pdf'
import {gilroyMediumBase64, gilroySemiBoldBase64} from '../pdf/font-pdf'
...
export default {
  ...
  methods: {
    ...
    //     scale: 1.5,        // ← уменьшите с 2 до 1.5 или 1
    //     useCORS: true,
    //     logging: false
    //   });
    //
    //   const imgData = canvas.toDataURL('image/jpeg', 0.92); // ← JPEG вместо PNG
    //
    //   const pdf = new jsPDF('p', 'pt', 'a4');
    //   const imgWidth = 595;
    //   const imgHeight = (canvas.height * imgWidth) / canvas.width;
    //
    //   pdf.addImage(imgData, 'JPEG', 0, 0, imgWidth, imgHeight); // ← 'JPEG'
    //   pdf.save('ipoteka-raschet.pdf');
    // },
    ...
    exportToPDF() {
      ...
      const docDefinition = {
        ...
        content: [
          ...
          this.createSectionBox(this.createMortgageSection(), 272),
//          this.createSectionBox(this.createRepaymentSection(), 255),
    ...
    createRepaymentSection() {
      return {
        stack: [
          { text: 'Данные о погашении', style: 'sectionTitleRepayment' },
    ...
    createRepaymentBlock() {
      ...
      const leftStackRow1 = [
        { text: 'Вы закроете ипотеку', style: 'labelGrey' },
        {
      ...
      const row1 = {
        columns: [
          { width: '*', stack: leftStackRow1 },
          {
            width: '*',
            stack: [
              { text: 'Ежемесячный платеж', style: 'labelGrey' },
        ...
        columns: [
          {
            width: '*',
            stack: [
              { text: 'Основной долг', style: 'labelGrey' },
      ...
      const row3Columns = [
        {
          width: '*',
          stack: [
            { text: 'Всего заплатите', style: 'labelGrey' },
      ...
      if (hasEarly) {
        row3Columns.push({
          width: '*',
          stack: [
            { text: 'Экономия', style: 'labelGrey' },
            { text: `${this.$options.filters.format_decimal(ri.savings)} ₽`, style: 'repaymentValueGreenKey' }
          ]
        });
      }
    ...
    createEarlyRepaymentBlock(product) {
      ...
      const BADGE2A_WIDTH = this.converPx(140); // "Уменьшение платежа" (18 символов, но шире из-за букв)
      ...
      return {
        stack: [
          ...
          // Содержимое
          {
            ...
            margin: [0, this.converPx(16), 0, this.converPx(16)] //внутренние отступы блока досрочное погашение padding
          }
        ],
        unbreakable: true,
        margin: [0, 0, 0, 0]
      ...
      return {
        stack: [
          ...
          },
          {
            ...
            margin: [this.converPx(16), 0, this.converPx(16), this.converPx(8)] //отступы ряда блока обычного пплатежа
          },
          {
            columns: [
              {
            ...
            margin: [this.converPx(16), 0, this.converPx(16), 0] //отступы ряда блока обычного пплатежа
          },
          {
            canvas: [
              {
...
//       pdfMake.fonts = {
//         Gilroy: {
//           normal: gilroyMediumBase64,      // Medium → normal
//           bold: gilroySemiBoldBase64,      // SemiBold → bold
//           italics: gilroyMediumBase64,
//           bolditalics: gilroySemiBoldBase64
//         }
//       };
//
//       console.log('First 10 chars:', gilroyMediumBase64.substring(0, 10)); // должно быть "AAEAAA"
//       console.log('Last 10 chars:', gilroyMediumBase64.substring(gilroyMediumBase64.length - 10)); // не должно быть "\n" или " "
//       console.log('Contains newline:', gilroyMediumBase64.includes('\n')); // должно быть false
//       console.log('Length:', gilroyMediumBase64.length); // должно быть > 500000
//
//
//       const docDefinition = {
//         pageSize: 'A4',
...
// },
  ...
  computed: {
    formattedTerm() {
      const unit = this.termMonthOrYear;
      let num;
      if (unit==='year') {
        num = this.loanTerm / 12
      }
      else if (unit==='month') {
        num = this.loanTerm
      }

      // Словарь для перевода и склонения
      const words = {
      ...
      const getForm = (n, forms) => {
        if (n % 10 === 1 && n % 100 !== 11) return forms[0]; // 1, 21, 31...
        if (n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 10 || n % 100 >= 20)) return forms[1]; // 2–4, 22–24...
        return forms[2]; // остальные случаи
      };

      return `${num} ${getForm(num, forms)}`;
    }
  },
  watch: {
    screenWidth() {
